...
Reads what the research tools already produced in the project folder:
  KEYWORD-INTELLIGENCE.json   (ranked keywords + Amazon-opportunity/trend scores)
...
def generate(folder, seed=""):
    ki = _load(folder, "KEYWORD-INTELLIGENCE.json") or {}
    ...
    kws = [r["keyword"] for r in ki.get("top", [])] or ([seed] if seed else [])
    if not kws:
        return {"ok": False, "reason": "no keywords — run keyword intelligence first (need KEYWORD-INTELLIGENCE.json)"}
    primary = kws[0]
    gaps = gapd.get("gaps", [])
    ...
    return {"ok": True, "listing": listing, "brief": brief,
